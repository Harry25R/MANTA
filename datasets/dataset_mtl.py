...
class Generic_WSI_MTL_Dataset(Dataset):
	def __init__(self,
		...
		if self.stain_level:
			self.patient_dict = self.build_patient_dict_by_stain()
		elif self.patient_level:
	...
	def build_patient_dict_by_stain(self):
		...
		for patient in patient_cases:
			slide_ids = dict(list(slide_cases.loc[patient].groupby('stain_type')['slide_id']))
			for stain_type in slide_ids.keys():
				stain_ids = slide_ids[stain_type]
				if isinstance(stain_ids, str):
					stain_ids = np.array(stain_ids).reshape(-1)
				else:
					stain_ids = stain_ids.values
				slide_ids[stain_type] = stain_ids
	...
	def get_split_from_df(self, all_splits=None, split_key='train', split=None):
		...
		if len(split) > 0:
			...
			split = Generic_Split(df_slice, data_dir=self.data_dir, num_classes=self.num_classes, label_cols=self.label_cols, patient_level = self.patient_level, stain_level = self.stain_level)
		else:
			split = None
	...
	def return_splits(self, from_id=True, csv_path=None):
		...
		if from_id:
			if len(self.train_ids) > 0:
				...
				train_split = Generic_Split(train_data, data_dir=self.data_dir, num_classes=self.num_classes, label_cols=self.label_cols, patient_level = self.patient_level, stain_level = self.stain_level)
			...
			if len(self.val_ids) > 0:
				...
				val_split = Generic_Split(val_data, data_dir=self.data_dir, num_classes=self.num_classes, label_cols=self.label_cols, patient_level = self.patient_level, stain_level = self.stain_level)
			...
			if len(self.test_ids) > 0:
				...
				test_split = Generic_Split(test_data, data_dir=self.data_dir, num_classes=self.num_classes, label_cols=self.label_cols, patient_level = self.patient_level, stain_level = self.stain_level)
...
class Generic_MIL_MTL_Dataset(Generic_WSI_MTL_Dataset):
	...
	def __getitem__(self, idx):
		...
		if self.stain_level:
			...
			stain_id_dict = self.patient_dict[case_id]
			...
			if not self.use_h5:
				...
				for stain in stain_id_dict.keys():
					features_stain = []
					slide_ids = stain_id_dict[stain]

					for slide_id in slide_ids:
						full_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id))
						slide_features = torch.load(full_path)
						features_stain.append(slide_features)

					features_stain = torch.cat(features_stain, dim = 0)
					features_all[stain] = features_stain

				return features_all['H+E'], features_all['Jones'], features_all['Trichrome'], features_all['PAS'], label_task1, label_task2, label_task3
				...
				for slide_id in slide_ids:
					...
					with h5py.File(full_path,'r') as hdf5_file:
						...
						features_list.append( silide_features_t )
...
class Generic_Split(Generic_MIL_MTL_Dataset):
	def __init__(self, slide_data, data_dir=None, num_classes=2, label_cols=None, patient_level=False, stain_level=False, fusion=""):
		self.use_h5 = False
		self.slide_data = slide_data
		self.data_dir = data_dir
